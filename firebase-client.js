...
async function loadCore() {
  if (!isFirebaseConfigured()) throw new Error("Firebase non configurato in firebase-config.js.");
  if (!corePromise) {
...
async function loadAuth(app) {
  if (!authPromise) {
    authPromise = import(sdkUrl("auth")).then((authApi) => ({ authApi, auth: authApi.getAuth(app) }));
  }
  return authPromise;
}
...
export async function getFirebaseServices({ includeAuth = false } = {}) {
  ...
  return includeAuth ? { ...core, ...(await loadAuth(core.app)) } : core;
}
